...
RAW_LINK_RE = re.compile(r'\[[^\]]*\]\(\s*(?:<([^>]+\.md)>|([^)\s]+\.md))\s*\)', re.IGNORECASE)
NO_MATERIAL_HEADING_RE = re.compile(r"^## \[[^\]]*\]\s*ingest\s*\|\s*no material:\s*(.+?)\s*$", re.IGNORECASE)
...
def raw_links_of(article_text: str) -> list[str]:
    ...
    for line in parse_document(article_text).header:
        if re.match(r"^>\s*Raw:", line.strip()):
            for match in RAW_LINK_RE.finditer(line):
